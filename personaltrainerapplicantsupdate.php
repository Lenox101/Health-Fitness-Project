...
if (isset($_POST['submit'])) {
    //Retrieve user id
    $personaltrainerapplicantsid = $_POST['personaltrainerapplicantsid'];
    // Retrieve user input
    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $status = $_POST['status'];
    ...
    $sql = "UPDATE personaltrainerapplicants SET FirstName = '$fname', LastName = '$lname', status = '$status', Weight = '$weight',Contact = '$contact', Gender = '$gender'
            WHERE Transaction_id='$personaltrainerapplicantsid'";
...
<body>
    ...
    <form id="editForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST" >
        <!----Get user ID----->
        <p>Transaction ID: </p>
        <input type="text" name="personaltrainerapplicantsid" id="personaltrainerapplicantsid" value="<?php echo htmlspecialchars(isset($_GET['personaltrainerapplicantsid']) ? $_GET['personaltrainerapplicantsid'] : ''); ?>" readonly><br>

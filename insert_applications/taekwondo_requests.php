...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Retrieve MemberId from the members_tbl based on the contact information
    ...
    if ($memberResult->num_rows > 0) {
        ...
        // Check if the member has already applied to this Taekwondo instructor
        $checkStmt = $conn->prepare("SELECT * FROM taekwondoapplicants WHERE `MemberId` = ? AND `Instructor Id` = ?");
        ...
        if ($checkResult->num_rows > 0) {
            ...
            // Preparing the SQL statement to insert into the taekwondoapplicants table
            $stmt = $conn->prepare("INSERT INTO taekwondoapplicants (`MemberId`, `Instructor's First Name`, `Instructor's Last Name`, `Instructor Id`, `FirstName`, `LastName`, `Weight`, `Contact`, `Email`, `Gender`, `amount`, `status`) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

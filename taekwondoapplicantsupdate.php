...
if (isset($_POST['submit'])) {
    //Retrieve user id
    $taekwondoapplicantsid = $_POST['taekwondoapplicantsid'];
    // Retrieve user input
    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $status = $_POST['status'];
    ...
    $sql = "UPDATE taekwondoapplicants SET FirstName = '$fname', LastName = '$lname', status = '$status', Weight = '$weight',Contact = '$contact', Gender = '$gender'
            WHERE Transaction_id='$taekwondoapplicantsid'";
...
<body>
    ...
    <form id="editForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST">
        <!----Get user ID----->
        <p>Transaction ID: </p>
        <input type="text" name="taekwondoapplicantsid" id="taekwondoapplicantsid" value="<?php echo htmlspecialchars(isset($_GET['taekwondoapplicantsid']) ? $_GET['taekwondoapplicantsid'] : ''); ?>" readonly><br>

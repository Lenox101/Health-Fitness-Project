...
if (isset($_POST['submit'])) {
    //Retrieve user id
    $nutritionistapplicantsid = $_POST['nutritionistapplicantsid'];
    // Retrieve user input
    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $status = $_POST['status'];
    ...
    $sql = "UPDATE nutritionistapplicants SET FirstName = '$fname', LastName = '$lname', status = '$status', Weight = '$weight',Contact = '$contact', Gender = '$gender'
            WHERE Transaction_id='$nutritionistapplicantsid'";
...
<body>
    ...
    <form id="editForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST">
        <!----Get user ID----->
        <p>Transaction ID: </p>
        <input type="text" name="nutritionistapplicantsid" id="nutritionistapplicantsid" value="<?php echo htmlspecialchars(isset($_GET['nutritionistapplicantsid']) ? $_GET['nutritionistapplicantsid'] : ''); ?>" readonly><br>

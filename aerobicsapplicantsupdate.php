...
if (isset($_POST['submit'])) {
    //Retrieve user id
    $aerobicsapplicantsid = $_POST['aerobicsapplicantsid'];
    // Retrieve user input
    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $status = $_POST['status'];
    ...
    $sql = "UPDATE aerobicsapplicants SET FirstName = '$fname', LastName = '$lname', status = '$status', Weight = '$weight',Contact = '$contact', Gender = '$gender'
            WHERE Transaction_id='$aerobicsapplicantsid'";
...
<body>
    ...
    <form id="editForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST">
        <!----Get Transaction ID----->
        <p>Transaction ID: </p>
        <input type="text" name="aerobicsapplicantsid" id="aerobicsapplicantsid" value="<?php echo htmlspecialchars(isset($_GET['aerobicsapplicantsid']) ? $_GET['aerobicsapplicantsid'] : ''); ?>" readonly><br>
